...
public:
    vector<int> successfulPairs(vector<int>& spells, vector<int>& potions, long long k){
        ...
        int n_p = potions.size(), n_s = spells.size();
        vector<int> ret;
        for(int s : spells){
            if(potions.back() * static_cast<long long>(s) < k){
                ret.push_back(0);
                continue;
            }
            else{
                int low = 0, high = n_p - 1;
                while(low <= high){
                    int mid = low + (high - low)/2;
                    if(static_cast<long long>(potions[mid]) * s >= k){
                        high = mid - 1;
                    }
                    else{
                        low = mid + 1;
