...
public:
    int findRotateSteps(string ring, string key) {
        int r_n = ring.size(), k_n = key.size(), ret = INT_MAX;
        unordered_map<int, vector<int>> pos;
        vector<vector<int>> cost(k_n, vector<int>(r_n, INT_MAX));
        for(int i = 0; i < r_n; i++) pos[ring[i] - 97].push_back(i);
        for(const auto& p : pos[key[0] - 97]) cost[0][p] = min(p, r_n - p) + 1;
        for(int i = 1; i < k_n; i++){
            for(int j : pos[key[i] - 97]){
                for(int k : pos[key[i-1] - 97]){
                    cost[i][j] = min(cost[i][j], cost[i-1][k] + min(abs(j-k), r_n-abs(j-k))+1);
                }
            }
        }
        for(const auto& p : pos[key[k_n - 1] - 97]) ret = min(ret, cost[k_n - 1][p]);
        return ret;
    }
};
