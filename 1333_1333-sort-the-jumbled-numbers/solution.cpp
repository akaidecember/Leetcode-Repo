...
public:
    vector<int> sortJumbled(vector<int>& mapping, vector<int>& nums) {
        int var, new_num, n = nums.size();
        vector<pair<int,int>> nums2;
        for(int i = 0; i < n; i++){
            int temp = nums[i];
            new_num = 0;
            var = 1;
            if (nums[i] == 0) new_num = mapping[0]; 
            else{
                while(temp){
                    int temp2 = temp % 10;
                    temp /= 10;
                    new_num += mapping[temp2] * var;
                    var *= 10;
                }
            }
            nums2.push_back({nums[i], new_num});
        }
        stable_sort(nums2.begin(), nums2.end(), [](const pair<int,int> &a, const pair<int,int> &b){
            return a.second < b.second;
        });
        for(int i = 0; i < n; i++){
            nums[i] = nums2[i].first;
        }
        return nums;
    }
};
