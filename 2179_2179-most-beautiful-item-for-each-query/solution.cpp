...
public:
    vector<int> maximumBeauty(vector<vector<int>>& items, vector<int>& queries) {
        int n = queries.size();
        vector<int> ret(n);
        vector<pair<int,int>> query_pair(n);
        
        transform(queries.begin(), queries.end(), query_pair.begin(), 
            [i = 0](int query) mutable {
                return make_pair(query, i++);
            });
        ...
        for(auto& [q, i] : query_pair){
            while(j < items.size() and items[j][0] <= q){
