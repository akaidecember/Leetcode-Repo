...
class Solution {
public:
    void helper(string &s, vector<string>& wordDict, vector<string> &ret, string &sent, int idx){
        if(idx == s.size()){
            if(sent.size() > 0)
                sent.pop_back();
            ret.push_back(sent);
        ...
        for(int i = idx; i < n; i++){
            sent += s[i];
            if(find(wordDict.begin(), wordDict.end(), sent) != wordDict.end()){
                string newStr = current + sent + " ";
                helper(s, wordDict, ret, newStr, i+1);
            }
        }
    }
