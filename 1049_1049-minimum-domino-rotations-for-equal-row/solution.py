...
class Solution:
    def minDominoRotations(self, top: List[int], bottom: List[int]) -> int:
        ...
        n = len(top)
        def check(candidate):
            ret_top = ret_bottom = 0
            for i in range(n):
                if top[i] != candidate and bottom[i] != candidate:
                    return float('inf')
                elif top[i] != candidate:
                    ret_top += 1
                elif bottom[i] != candidate:
                    ret_bottom += 1
            return min(ret_top, ret_bottom)

        ret = min(check(top[0]), check(bottom[0]))
