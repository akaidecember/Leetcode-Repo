# Link: https://leetcode.com/problems/apply-substitutions/description
# Description
# You are given a
# replacements
# mapping and a
...
class Solution:
    def applySubstitutions(self, replacements: List[List[str]], text: str) -> str:
        n = len(replacements)
        nodes = {k:v for k,v in replacements}
        graph = {node : set() for node in nodes}
        indegree = {node : 0 for node in nodes}

        for i in range(n):
            node, val = replacements[i]
            for j in range(n):
                if j != i:
                    curr_node, curr_val = replacements[j]
                    placeholder = f"%{node}%"
                    if placeholder in curr_val:
        ...
        while q:
            ...
            if node not in vis:
                ...
                for nei in graph[node]:
                    indegree[nei] -= 1
                    if indegree[nei] == 0 and nei not in vis:
                        q.append(nei)

        for i in range(n):
            node, val = topo[i], nodes[topo[i]]
            for j in range(i):
                prev_node, prev_val = topo[j], nodes[topo[j]]
                if node in graph[prev_node]:
                    pattern = f"%{prev_node}%"
                    val = val.replace(pattern, prev_val)
            nodes[node] = val

        ret = text
        for node in topo:
            ret = ret.replace(f"%{node}%", nodes[node])
