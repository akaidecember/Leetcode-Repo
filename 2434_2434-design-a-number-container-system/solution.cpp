...
public:
    unordered_map<int, priority_queue<int, vector<int>, greater<int>>> ret;
    ...
    void change(int index, int number) {
        if(mp.count(index)){
            ...
            ret[prev].push(INT_MAX);
        }
        ret[number].push(index);
        mp[index] = number;
    }
    
    int find(int number) {
        while (!ret[number].empty() && mp[ret[number].top()] != number) {
            ret[number].pop();
        }
        return ret[number].empty() ? -1 : ret[number].top();
    }
};
