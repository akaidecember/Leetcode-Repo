...
public:
    void dfs(vector<vector<int>>& image, int sr, int sc, int color, int colorBefore){
        image[sr][sc] = color;
        int m = image.size(), n = image[0].size();
        for(const auto& d : dirs){
            int x = sr + d[0];
            int y = sc + d[1];
            if(x >= 0 && x < m && y >= 0 && y < n && image[x][y] == colorBefore && image[x][y] != color){
                dfs(image, x, y, color, colorBefore);
            }
        }
    }
