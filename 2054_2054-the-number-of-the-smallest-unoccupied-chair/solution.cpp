...
public:
    int smallestChair(vector<vector<int>>& times, int targetFriend) {
        ...
        for(int i = 0; i < n; i++){
            int start = times[i][0], end = times[i][1], curr_friend = times[i][2];

            while(!occupied.empty() and occupied.top().first <= start){
                empty_chairs.push(occupied.top().second);
                occupied.pop();
            }
            ...
            if(empty_chairs.empty()){
                ...
                chair_id = empty_chairs.top();
                empty_chairs.pop();
            }

            if(curr_friend == targetFriend){
                return chair_id;
            }

            occupied.push({end, chair_id});
        }
