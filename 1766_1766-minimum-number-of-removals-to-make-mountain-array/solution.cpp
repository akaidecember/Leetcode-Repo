...
class Solution {
public:
    void longestIncreasingSubsequence(const vector<int>& nums, vector<int>& lis) {
        vector<int> vec; 
        int n = nums.size();

        for(int i = 0; i < n; i++){
            int val = nums[i];
            auto pos = lower_bound(vec.begin(), vec.end(), val); 
    ...
    int minimumMountainRemovals(vector<int>& nums) {
        ...
        longestIncreasingSubsequence(vector<int>(nums.rbegin(), nums.rend()), lds);
        reverse(lds.begin(), lds.end());

        for(int i = 1; i < n-1; i++){
            max_val = max(max_val, (lis[i] > 1 and lds[i] > 1) ? lis[i] + lds[i] - 1 : 0);
        }
