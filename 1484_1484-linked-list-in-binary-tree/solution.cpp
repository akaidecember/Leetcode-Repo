...
class Solution {
public:
    bool sameTree(ListNode* head, TreeNode* root){
        if(!head){
            return true;
        }
        if(!root or root->val != head->val){
            return false;
        }
        return sameTree(head->next, root->left) or sameTree(head->next, root->right);
    }
