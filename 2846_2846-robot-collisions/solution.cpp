...
public:
    vector<int> survivedRobotsHealths(vector<int>& positions, vector<int>& healths, string directions){
        ...
        vector<vector<int>> v;
        for (int i = 0; i < n; i++){
            // Note: position 0 - pos, 1 - health, 2 - dirs.
            v.push_back({positions[i], healths[i], directions.at(i) == 'R' ? 1 : -1, i});
        }
        ...
        for (int i = 0; i < v.size(); i++){
            if(stack.empty() || stack.back()[2] == v[i][2] || stack.back()[2] == -1){
                stack.push_back(v[i]);
            } 
            else{
                while(!stack.empty() && stack.back()[2] == 1 && v[i][2] == -1){
                    if(stack.back()[1] == v[i][1]){
                        stack.pop_back();
                        v[i][1] = -1;
                        break;
                    }
                    else if(stack.back()[1] < v[i][1]){
                        v[i][1]--;
                        stack.pop_back();
                    }
                    else{
        ...
        vector<int> ret;
        for (const auto& s : stack) ret.push_back(s[1]);
        return ret;
    }
};
