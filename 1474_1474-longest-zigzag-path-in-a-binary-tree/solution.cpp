...
class Solution {
public:
    void max_z(TreeNode *node, bool go_l, int stp, int &ret){
        if(!node){
            return;
        }
        ret = max(ret, stp);
        if(go_l){
            max_z(node->left, false, stp + 1, ret);
            max_z(node->right, true, 1, ret);
        }
        else{
            max_z(node->left, false, 1, ret);
            max_z(node->right, true, stp + 1, ret);
        }
    }

    int longestZigZag(TreeNode* root) {
        int ret = 0;
        max_z(root, false, 0, ret);
        max_z(root, true, 0, ret);
        return ret;
    }
};
