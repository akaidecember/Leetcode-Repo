...
class Solution {
public:
    int helper(int idx, int curr_or, vector<int>& nums, int& max_or){
        if(idx < 0){
            return (curr_or == max_or) ? 1 : 0;
        }

        return helper(idx - 1, curr_or, nums, max_or) + helper(idx - 1, curr_or | nums[idx], nums, max_or);
    }

    int countMaxOrSubsets(vector<int>& nums) {
        int max_or = accumulate(nums.begin(), nums.end(), 0, bit_or<>());
        
        return helper(nums.size() - 1, 0, nums, max_or);
    }
};
